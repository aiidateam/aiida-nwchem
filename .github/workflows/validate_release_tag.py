...
if __name__ == '__main__':
    ...
    assert args.GITHUB_REF.startswith(REF_PREFIX), f'GITHUB_REF should start with "{REF_PREFIX}": {args.GITHUB_REF}'
    tag_version = args.GITHUB_REF[len(REF_PREFIX):]
    package_version = get_version_from_module(Path('src/aiida_nwchem/__init__.py').read_text(encoding='utf-8'))
